...
export function clearMessages(room: Room): void {
  ...
  void store.clearRoomSeen(room.id);
  void store.publishRoom({ kind: "clear", roomId: room.id });
}

/**
 * Update the read position for a participant. The in-memory value changes
 * synchronously (the UI reads it right away); the durable copy is a single
 * HSET via the store rather than a whole-room rewrite.
 */
export function setParticipantLastSeen(room: Room, participantId: string, messageId: string): void {
  ...
  void store.setParticipantSeen(room.id, participantId, messageId);
}
...
function applySnapshot(room: Room, snap: RoomSnapshot): void {
  ...
  const next = new Map(
    snap.participants.map((p) => {
      // A snapshot generated by another instance may carry a stale (or no)
      // lastSeenMessageId; keep our locally-known read position where we have
      // a fresher idea than the snapshot provides.
      const local = room.participants.get(p.id);
      const lastSeen = p.lastSeenMessageId ?? local?.lastSeenMessageId;
      return [p.id, lastSeen ? { ...p, lastSeenMessageId: lastSeen } : { ...p }];
    }),
  );
