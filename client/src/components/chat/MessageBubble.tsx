    ...
    >
      ...
      {replyEnabled && (
        <button
          ...
          aria-label="Reply"
          className="absolute right-0.5 top-1/2 z-10 flex -translate-y-1/2 items-center justify-center rounded-full border border-base-border2 bg-base-raised p-1.5 text-ink-muted opacity-0 shadow-md transition-opacity hover:text-accent group-hover:opacity-100 focus-visible:opacity-100"
        >
      ...
      >
        ...
        {hasMedia && message.attachment?.type === "voice" ? (
          ...
          <div className={`cryo-in flex min-w-0 max-w-full items-end gap-1.5 ${emojiOnly ? "flex-col items-start" : ""}`}>
            {emojiOnly ? (
              <>
              ...
              <div
                className={`min-w-0 rounded-bubble px-3.5 py-2 pb-1.5 text-[15px] leading-relaxed ${
                  mine
